...
def main(page: ft.Page):
    ...
    def mostrar_login():
        ...
        def entrar(_=None):
            ...
            state["user"] = nome
            state["perfil"] = (str(match.get("perfil") or "user").strip().lower() or "user")
            # Pré-cache paralelo de dados comuns para navegação mais rápida
            try:
                client.call("get_obras",              ttl=600)
                client.call("get_consumiveis",        ttl=600)
                client.call("get_maquinas",           ttl=300)
                client.call("get_catalogo_materiais", ttl=600)
            except Exception:
    ...
    def mostrar_maquinas():
        page.clean()
        try:
            obras_rows = client.call("get_obras", ttl=600)
            maqs_rows = client.call("get_maquinas", ttl=300)
        except Exception as e:
    ...
    def mostrar_pedido():
        ...
        try:
            obras_rows = client.call("get_obras", ttl=600)
            cons_rows = client.call("get_consumiveis", ttl=600)
            try:
                mat_rows = client.call("get_catalogo_materiais", ttl=600)
            except Exception:
                mat_rows = []
    ...
    def mostrar_historico():
        page.clean()
        is_admin = state.get("perfil") == "admin"
        try:
            params = {"user": state["user"] or ""}
            if is_admin:
                params["all"] = "1"   # admin vê todos
            rows = client.call("get_historico", params, ttl=10)
        except Exception as e:
            snack(f"Erro: {e}", COR_ERRO)
            return mostrar_portal()

        titulo = "Histórico (TODOS — admin)" if is_admin else "Histórico"
        if not rows:
            ...
            for r in rows:
                ...
                texto_item = f"{item}"
                if detalhes:
                    texto_item += f"  ({detalhes})"
                texto_item += f"  ×{qtd}"

                rodape_partes = [obra]
                if is_admin and utilizador:
                    rodape_partes.append(f"👤 {utilizador}")
                rodape = "   ·   ".join([p for p in rodape_partes if p])

                body_items.append(ft.Container(
                    bgcolor="white", border_radius=8, padding=12,
                    content=ft.Column([
                        ft.Row(linha_topo, alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                        ft.Text(texto_item, size=14, weight="bold"),
                        ft.Text(rodape, size=12, color="grey"),
                    ], spacing=3),
                ))
            body = ft.Column(body_items, spacing=8)

        page.add(
            top_bar(titulo, back=lambda e: mostrar_portal()),
